...
st.set_page_config(page_title="Clubhouse Canteen Pre-Order", page_icon="🍲", layout="centered")
...
# Helper: Generate UPI QR Code
def get_upi_qr(upi_id: str, payee_name: str, amount: float, order_id: str):
    upi_url = f"upi://pay?pa={upi_id}&pn={payee_name}&am={amount}&tn=Order_{order_id}&cu=INR"
    qr = qrcode.make(upi_url)
    buf = io.BytesIO()
    qr.save(buf, format="PNG")
    return buf.getvalue()

tab_order, tab_kitchen = st.tabs(["🛒 Place Pre-Order", "👨‍🍳 Kitchen Dashboard"])
...
with tab_order:
    ...
    with st.expander("Resident & Delivery Details", expanded=True):
        c1, c2 = st.columns(2)
        flat_no = c1.text_input("Flat / Door Number*")
        resident_name = c2.text_input("Your Name*")
        phone = c1.text_input("Mobile Number*")
        slot = c2.selectbox("Pickup Time Slot*", [
    ...
    try:
        ...
        st.error("Unable to load menu. Please check the Google Sheet connection.")
    ...
    if total_bill > 0 and flat_no and resident_name and phone:
        # Create an Order ID using timestamp
        order_id = f"ORD-{datetime.now().strftime('%H%M%S')}"
        
        # Payment QR (Set your friend's canteen UPI ID here)
        st.subheader("Scan & Pay via UPI")
        st.image(get_upi_qr("canteen@upi", "Clubhouse Canteen", total_bill, order_id), width=180)
        st.caption(f"Ref Note: `{order_id}` | Flat: `{flat_no}`")
        ...
        if st.button("Confirm & Place Order", type="primary", use_container_width=True):
            ...
            try:
                # Read current orders, append new order, and write back
                existing_orders = load_orders()
                updated_orders = pd.concat([existing_orders, new_record], ignore_index=True)
                conn.update(worksheet="orders", data=updated_orders)
                st.success(f"Order #{order_id} recorded! See you tomorrow at {slot}.")
                st.balloons()
            except Exception as ex:
                st.error(f"Error saving order: {ex}")
    elif total_bill > 0:
        st.info("Please fill in Flat Number, Name, and Phone above to complete order.")
    ...
    if admin_pin == "5678":
        try:
            ...
            if orders_df.empty or "items" not in orders_df.columns:
                ...
                # Consolidate kitchen requirements
                item_counts = {}
                ...
                if item_counts:
                    summary_df = pd.DataFrame(list(item_counts.items()), columns=["Item", "Total Quantity Required"])
                    st.dataframe(summary_df, use_container_width=True)
                
                st.subheader("All Scheduled Orders")
                st.dataframe(active_orders, use_container_width=True)
        except Exception as e:
            st.error(f"Error reading orders database: {e}")
    elif admin_pin:
